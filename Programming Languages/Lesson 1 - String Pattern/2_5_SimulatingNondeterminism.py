...
# fill in your code here 
    if string == "":
        ...
        if (current, string[0]) in edges:
            result = False
            for item in edges[(current, string[0])]:
                result = result or nfsmsim(string[1:], item, edges, accepting)
            return result
        else:
            return False
 
# Once accept then return, don't like my solution needs to calculate all the possibilities       
def nfsmsim_Offical_Answer(string, current, edges, accepting): 
    if string == "":
        return current in accepting
    else:
        letter = string[0:1]
        if (current. letter) in edges:
            ...
            for newstate in newstates:
                if nfsmsim_Offical_Answer(remainder, newstate, edges, accepting): 
                    return True 
            return False
