...
def nfsmaccepts(current, edges, accepting, visited): 
    ...
    if current in accepting:
        return ""
    if visited != None and current in visited:
        return None
        ...
        if k[0] == current:
            ...
            for value in v:
                ...
                if value not in visited:
                    temp = nfsmaccepts(value, edges, accepting, visited)
                    if temp != None:
                        return k[1] + temp
                    #result += nfsmaccepts(value, edges, accepting, visited)
                #else:
                    #result = result[1:]
            return None
            
# Offical Answer, also 13 line of code:
def nfsmaccepts_Offical_Answer(current, edges, accepting, visited):
        ...
        for edge in edges:
            if edge[0] == current:
                for newstate in edges[edge]:
                    foo = nfsmaccepts_Offical_Answer(newstate, edges, accepting, newvisited)
                    if foo != None:
                        return edge[1] + foo
        return None
...
#print str(nfsmaccepts(2, edges4, accepting4, []))
#print str(nfsmaccepts(1, edges3, accepting3, []))
print str(nfsmaccepts(1, edges, accepting, []))
print str(nfsmaccepts(1, edges, [4], []))
print str(nfsmaccepts(1, edges2, accepting2, []))
print str(nfsmaccepts(1, edges2, [1], []))
...
print "Test 5: " + str(nfsmaccepts_Offical_Answer(1, {(1, 'a'): [1]}, [1], []))
# This problem spend me days, this are the experiences get from it:
# 1. Things are more clear when I started to focus on Test 3 and return None. Actually Test 3 is just a simple case. I mixup return ""
#    and return None at the beginning.
# 2. Actually return "" is a accepting state, return None is a reject state.
# 3. Error will be raisen if you concatenate a string and a None, but a None instance could be compared with None so that we can avoid
