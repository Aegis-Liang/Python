...
def calculate_error(num_steps):
    ...
    for i in range(num_steps):
        x[i + 1] = x[i] + v[i] * h
        v[i + 1] = v[i] + acceleration(x[i]) * h
        
    error = numpy.linalg.norm(x[-1] - x[0]) # My solution use (num_steps-1) for index, but the x has size of num_steps + 1, that's why it's wrong. 
