...
def best_hand(hand):
    ...
    #return max(map(hand_rank, hands)) # cannot come up with a way to use map and max function
    maxrank = (0,)
    maxhand = []
    for h in hands:
        this_hand_rank = hand_rank(h)
        if this_hand_rank > maxrank:
            maxrank = this_hand_rank
            maxhand = h
    return maxhand            
